#ifndef _14_26_STRVEC_H_
#define _14_26_STRVEC_H_
...
public:
    ...
    // 下标运算符的返回值通常为所指对象的引用，从而可以出现在赋值运算符的任一端
...
bool operator<(const StrVec &lhs, const StrVec &rhs) {
    auto lbeg = lhs.elements;
    auto rbeg = rhs.elements;

    for (; (lbeg != lhs.first_free) && (rbeg != rhs.first_free); ++lbeg, ++rbeg) {
        if (*lbeg < *rbeg) return true;
        else if (*lbeg > *rbeg) return false;
    }

    return (lbeg == lhs.first_free) && (rbeg != rhs.first_free);
}
