...
// 接受一个映射规则的文件流，读取映射规则
// 返回一个映射的 map table
std::unordered_map<std::string, std::string> buildmap(std::fstream &mapfile);

// 转换函数
const std::string& transformer(const std::string& s, const std::unordered_map<std::string, std::string> &map);

std::unordered_map<std::string, std::string> buildmap(std::fstream& mapfile)
{
    std::string key;
    std::string value;
    std::unordered_map<std::string, std::string> map_table;
